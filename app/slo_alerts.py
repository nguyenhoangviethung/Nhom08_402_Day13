...
def evaluate_slos(metrics: dict[str, Any], config_path: Path = SLO_CONFIG_PATH) -> dict[str, Any]:
    ...
    for sli_name, spec in slis.items():
        ...
        current_value = _as_float(metrics.get(metric_key, 0.0))
...
def _evaluate_condition(condition: str, metrics: dict[str, Any]) -> tuple[bool, str]:
    ...
    if match:
        ...
        value = _as_float(metrics.get(metric, 0.0))
        ...
        return operations.get(op, False), f"{metric}={round(value, 4)}, threshold={threshold}, window={match.group('window')}"
    ...
    if match:
        ...
        return value > threshold, (
            f"{metric}={round(value, 4)}, baseline={baseline}, factor={factor}, threshold={round(threshold, 4)}, "
